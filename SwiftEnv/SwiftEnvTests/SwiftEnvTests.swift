...
class SwiftEnvTests: XCTestCase {
    func testExtract_required_string_is_found_no_error() {
        let env = ["asdf": "fdsa"]
        let v = try! ValueParser().extractFrom(env, key: "asdf").required()
    ...
    func testExtract_required_string_is_not_found_error() {
        let env = ["asdf": "fdsa"]
        do {
            let _ = try ValueParser().extractFrom(env, key: "qwer").required()
    ...
    func testExtract_required_int_is_found() {
        let env = ["asdf": "1"]
        let value: Int = try! ValueParser().extractFrom(env, key: "asdf").asInt().required()
    ...
    func testExtract_required_int_is_not_found_error() {
        let env = ["asdf": "1"]
        do {
            let _ = try ValueParser().extractFrom(env, key: "qwer").asInt().required()
        } catch ExtractError.ValueMissing(let name) {
            XCTAssertEqual(name, "qwer")
            return
        } catch let err {
    ...
    func testExtract_required_int_is_not_int_error() {
        let env = ["asdf": "notint"]
        do {
            let _ = try ValueParser().extractFrom(env, key: "asdf").asInt().required()
    ...
    func testExtract_missing_value_replaced_by_default_value() {
        let env = ["asdf": "notint"]
        let value = try! ValueParser().extractFrom(env, key: "qwer").asInt().defaultValue(6)
    ...
    func testExtract_default_value_int_is_not_int_error() {
        let env = ["asdf": "notint"]
        do {
            let _ = try ValueParser().extractFrom(env, key: "asdf").asInt().defaultValue(12)
        } catch ExtractError.FormatError(let name, let value) {
            XCTAssertEqual(name, "asdf")
            XCTAssertEqual(value, "notint")
            return
